...
function delayQuerySelector (
  ...
  selectorStr: string,
  delayTime = 500
): Promise<Array<SelectorQuery.execObject>> {
  ...
  return new Promise(resolve => {
    delay(delayTime).then(() => {
      selector
        .select(selectorStr)
        .boundingClientRect()
        .exec((res: Array<SelectorQuery.execObject>) => {
          resolve(res)
        })
    })
  })
...
function delayGetScrollOffset ({ delayTime = 500 }): Promise<Array<SelectorQuery.execObject>> {
  return new Promise(resolve => {
    delay(delayTime).then(() => {
